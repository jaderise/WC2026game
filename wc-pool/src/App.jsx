...
// over time (a bump chart) and GAP to the current leader. Top 5 (by current total) get
// colors, the rest are grey; hover reveals a name, tap locks a 6th highlight. Highlight
...
function PointsChart({ rows, results }) {
  ...
  const data = useMemo(() => {
    ...
    const topNames = [...series].sort((a, b) => b.final - a.final).slice(0, 5).map((s) => s.name);
